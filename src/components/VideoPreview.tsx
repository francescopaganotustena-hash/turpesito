...
import { instagramReels, videos } from '../data';
import { enableInstagramVideoEmbeds, enableVideoPage } from '../config/featureFlags';
...
export function VideoPreview() {
  ...
  if (!enableVideoPage) return null;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 justify-items-center">
          {previewVideos.map((video) => (
            <Link
              ...
              className="group relative rounded-lg overflow-hidden w-full max-w-sm aspect-[9/16]"
            >
              <img
                src={video.thumbnail}
                alt={video.title}
